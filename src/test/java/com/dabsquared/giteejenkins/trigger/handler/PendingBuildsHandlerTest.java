...
import com.dabsquared.giteejenkins.gitee.hook.model.*;
import com.dabsquared.giteejenkins.gitee.hook.model.builder.generated.*;
...
import static com.dabsquared.giteejenkins.gitee.hook.model.builder.generated.CommitBuilder.commit;
import static com.dabsquared.giteejenkins.gitee.hook.model.builder.generated.MergeRequestObjectAttributesBuilder.mergeRequestObjectAttributes;
import static com.dabsquared.giteejenkins.gitee.hook.model.builder.generated.UserBuilder.user;
...
public class PendingBuildsHandlerTest {
    ...
    public void projectCanBeConfiguredToSendPendingBuildStatusWhenTriggered() throws IOException {
        Project project = freestyleProject("freestyleProject1", new GiteeCommitStatusPublisher(GITLAB_BUILD_NAME, false));

        GiteePushTrigger giteePushTrigger = gitLabPushTrigger(project);

        giteePushTrigger.onPost(pushHook(1, "branch1", "commit1"));

        verify(giteeClient).changeBuildStatus(eq(1), eq("commit1"), eq(BuildState.pending), eq("branch1"), eq(GITLAB_BUILD_NAME),
    ...
    public void workflowJobCanConfiguredToSendToPendingBuildStatusWhenTriggered() throws IOException {
        ...
        GiteePushTrigger giteePushTrigger = gitLabPushTrigger(workflowJob);
        giteePushTrigger.setPendingBuildName(GITLAB_BUILD_NAME);
    ...
    public void queuedMergeRequestBuildsCanBeCancelledOnMergeRequestUpdate() throws IOException {
        Project project = freestyleProject("project1", new GiteeCommitStatusPublisher(GITLAB_BUILD_NAME, false));
    ...
    private GiteePushTrigger gitLabPushTrigger(Project project) throws IOException {
        GiteePushTrigger giteePushTrigger = gitLabPushTrigger();
        project.addTrigger(giteePushTrigger);
        giteePushTrigger.start(project,true);
        return giteePushTrigger;
    }

    private GiteePushTrigger gitLabPushTrigger(WorkflowJob workflowJob) {
        GiteePushTrigger giteePushTrigger = gitLabPushTrigger();
    ...
    private GiteePushTrigger gitLabPushTrigger() {
    ...
    private Project freestyleProject(String name, GiteeCommitStatusPublisher giteeCommitStatusPublisher) throws IOException {
