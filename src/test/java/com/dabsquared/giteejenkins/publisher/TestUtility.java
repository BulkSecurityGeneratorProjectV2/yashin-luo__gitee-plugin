...
final class TestUtility {
    ...
    static void setupGitLabConnections(JenkinsRule jenkins, MockServerRule mockServer) throws IOException {
        ...
        connectionConfig.addConnection(new GiteeConnection(TestUtility.GITLAB_CONNECTION_V3, "http://localhost:" + mockServer.getPort() + "/gitee", apiTokenId, new V3GiteeClientBuilder(), false, 10, 10));
        connectionConfig.addConnection(new GiteeConnection(TestUtility.GITLAB_CONNECTION_V4, "http://localhost:" + mockServer.getPort() + "/gitee", apiTokenId, new V4GiteeClientBuilder(), false, 10, 10));
