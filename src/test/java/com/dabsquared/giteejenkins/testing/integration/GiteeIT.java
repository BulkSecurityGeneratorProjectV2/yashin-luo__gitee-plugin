...
import static com.dabsquared.giteejenkins.builder.generated.GitLabPushTriggerBuilder.gitLabPushTrigger;
import static com.dabsquared.giteejenkins.testing.gitee.rule.builder.generated.ProjectRequestBuilder.projectRequest;
...
public class GiteeIT {
    private static final String GITLAB_URL = "http://localhost:" + System.getProperty("gitee.http.port", "10080");

    @Rule
    public GiteeRule gitlab = new GiteeRule(GITLAB_URL, parseInt(System.getProperty("postgres.port", "5432")));
    ...
    public void buildOnPush() throws IOException, InterruptedException, GitAPIException {
        ...
        GiteePushTrigger trigger = gitLabPushTrigger().withTriggerOnPush(true).withBranchFilterType(BranchFilterType.All).build();
    ...
    public void buildOnMergeRequest() throws IOException, InterruptedException, GitAPIException {
        final OneShotEvent buildTriggered = new OneShotEvent();
        FreeStyleProject project = jenkins.createFreeStyleProject("test");
        GiteePushTrigger trigger = gitLabPushTrigger().withTriggerOnMergeRequest(true).withBranchFilterType(BranchFilterType.All).build();
        project.addTrigger(trigger);
        trigger.start(project, true);
        project.getBuildersList().add(new TestBuilder() {
            @Override
    ...
    public void buildOnNote() throws IOException, InterruptedException, GitAPIException {
        final OneShotEvent buildTriggered = new OneShotEvent();
        FreeStyleProject project = jenkins.createFreeStyleProject("test");
        GiteePushTrigger trigger = gitLabPushTrigger()
    ...
    private Pair<Integer, String> createGitLabProject(boolean addFeatureBranch, boolean withPushHook, boolean withNoteHook, boolean withMergeRequestHook) throws IOException, GitAPIException {
        ...
        // GiteeRule and to use the id from the project instance. However, due to a bug in GitLab (tested on 8.6.1)
